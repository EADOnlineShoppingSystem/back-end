...
const deleteCartByUserId = async (req,res)=>{
    try {
        const userId = req.user.id;
        const deleteCart = await cartServices.deleteCartByUserId(userId);
        res.status(200).json(deleteCart);
    } catch (error) {
        res.status(500).json({message:error.message})
    }
}
export default {addToCart,getCartDetailsByUserID,deleteCart,getAllQuantityByUsers,updateCartQuantity,deleteCartByUserId}